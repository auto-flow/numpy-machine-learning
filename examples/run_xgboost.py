...
from mylearn.tree_model.xgboost import XgbClassifier, XgbRegressor
...
for params in [
    dict(n_estimators=10, learning_rate=0.1, max_depth=3),
    dict(n_estimators=10, learning_rate=1, max_depth=3),
    dict(n_estimators=20, learning_rate=0.5, max_depth=3),
]:
    print("#" * 60)
    print(params)
    print("#" * 60)
    run_score(load_iris, XgbClassifier, "My", params)
    # run_score(load_iris, SklearnGradientBoostingClassifier, "Sklearn", params)
    run_score(load_boston, XgbRegressor, "My", params)
